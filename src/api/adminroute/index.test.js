...
test('GET /adminroutes/:id 401', async () => {
  const { status } = await request(app()).get(`${apiRoot}/${adminroute.id}`);
  expect(status).toBe(401);
});
...
test('PUT /adminroutes/:id 401', async () => {
  const { status } = await request(app()).put(`${apiRoot}/${adminroute.id}`);
  expect(status).toBe(401);
});
...
test('DELETE /adminroutes/:id 401', async () => {
  const { status } = await request(app()).delete(`${apiRoot}/${adminroute.id}`);
  expect(status).toBe(401);
});
